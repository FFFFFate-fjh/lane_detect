...
//    string filename = "data/04/image_0/list.txt"; //使用正则表达生成图片名字的list
    ...
    while( getline(ifile,tmp) )
    {
        ...
        cout << "Read from file: " << img_list[count] << endl;
        count++;
        tmp.clear();
    }
    ifile.close();

    int img_num=img_list.size()-4;
    cout<<"img num: "<<img_num<<endl;
    int img_begin=3;
    int img_end=img_list.size()-2;
    cout<<"img_end: "<<img_end<<endl;

    for(int i=img_begin;i<=img_end;i++)  //读图
    {
        char *addr_buf;
        int len = img_list[i].length();
        addr_buf =new char[len+1];
        strcpy(addr_buf,img_list[i].c_str());
        char img_buf[100];
...
//        Mat ROI_img = set_ROI(src_heq,ROI_x, ROI_y, 600, 369-ROI_y);
        Mat ROI_img2 = set_ROI(src_heq,75, 130, img_width-75, img_height-130);
        Mat ROI_img = set_ROI(src_heq,ROI_x, ROI_y, 760, img_height-ROI_y);
        Mat grad, abs_grad_x;
...
float ema1_init(float ema_input, int &ema_init_count, float &ema_init_total, float &ema_last, float &ema_init)
{
    ...
    if(ema_init_count!=0)
    {
//        float temp = ema_init_total/(float)(ema_init_limit-ema_init_count);
//        if((ema_init_total>10e-3&&ema_init_total<-10e-3)&&ema_input_f>1.3*temp||ema_input_f<0.7*temp)
//            return temp;
        ema_init_count--;
...
//                if(dev_tmp1[i]<mean+stddev && dev_tmp1[i]>mean-stddev)
                if(dev_tmp1[i]<mean+1 && dev_tmp1[i]>mean-stddev)
...
Mat opt_ROI(Mat src, float kl,float kr,float bl, float br )
{
    ...
    for(int y=0; y<img_height-ROI_y; y++)
    {
        ...
        int prop = (int)((float)src.cols*0.1);
        int prop2 = (int)((float)src.cols*0.2);
        xl = max((int)xl, prop);
        xr = min((int)xr, src.cols-prop);

        if((int)xl==prop || (int)xr == src.cols-prop)
        {
            for(int x=(int)xl+prop; x<(int)xr-prop; x++)
                dst.at<uchar>(y,x)=int_zero.val[0];
            for(int x=0; x<(int)xl-prop; x++)
                dst.at<uchar>(y,x)=int_zero.val[0];
            for(int x=(int)xr+prop; x<src.cols; x++)
                dst.at<uchar>(y,x)=int_zero.val[0];
        }
        else
        {
